...
class Shop extends Base {
        ...
        if (IS_POST) {
            ...
            if (!empty($IsOrder)) {
                ...
                $Result['notice'] = [
                    ...
                    'users_id' => $post['users_id'],
                    'order_id' => $post['order_id'],
                    'url' => ROOT_DIR . '/index.php?m=api&c=Ajax&a=sendNotice&_ajax=1',
                ];
    ...
    public function verify()
    {
        ...
        $where = [];
        if ($this->UsersConfigData['shop_open_verify_code'] == 1){
            $verify_code = input('param.verify_code/s');
            if (empty($verify_code)){
                $this->error('缺少必要参数！');
            }
            $where['verify_code'] = trim($verify_code);
        }else{
            $where['order_id'] = $order_id;
        }

        $verify_order = Db::name('shop_order_verify')->where($where)->find();
        if (empty($verify_order)){
            ...
            if ($r !== false){
                ...
                AddOrderAction($verify_order['order_id'], $users_id, 0, 3, 1, 1, '后台管理员员核销订单', '核销订单成功');
                $this->success('核销成功！');
            }
            $this->error('核销失败！');
        }
