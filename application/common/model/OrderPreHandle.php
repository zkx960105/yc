...
class OrderPreHandle extends Model
{
    ...
    public function eyou_shopOrderPreHandle($users_id = 0, $usersConfig = [])
    {
        // 参数处理
        $this->users_id = !empty($users_id) ? $users_id : 0;
        ...
        $this->eyou_dealerOrderSettlementHandle();
    }
    ...
    // 恢复商品库存
    private function restoreGoodsStock($order_id)
    {
        // 查询订单商品
        $where = [];
        // 查询条件-会员ID处理
        ...
        $field = 'a.product_id, a.num as product_num, a.data as product_data, b.value_id, b.spec_value_id';
        ...
        if (!empty($orderDetails)) {
            ...
            foreach ($orderDetails as $key => $value) {
                ...
                $value_id = !empty($productData['value_id']) ? $productData['value_id'] : 0;
